...
export default function Signup() {
  ...
  const inputStyles = (field: string) =>
    `w-full pl-11 pr-12 py-3 text-[0.875rem] text-text-main rounded-xl border bg-white dark:bg-[#13141a] text-[#1D1D18] dark:text-white transition-all duration-200 font-sans text-sm focus:outline-none hover:bg-[#FFF9F2] dark:hover:bg-[#FF8600]/5 hover:border-primary/50 ${errors[field]
  ...
  const labelStyles = (field: string, value: string) =>
    `absolute pointer-events-none transition-all duration-500 ease-in-out font-sans select-none ${focusedFields[field] || value !== ''
    ...
    return (
      <div className="w-full max-w-[489px] p-[50px] mx-auto bg-white dark:bg-[#181920] border border-[#DDE2E4] dark:border-[#2e303a]/60 rounded-2xl shadow-[10px_50px_50px_rgba(0,0,0,0.059)] dark:shadow-[10px_50px_50px_rgba(0,0,0,0.25)] transition-all duration-300">
        ...
        <div className="mt-8 text-left text-sm text-text-main dark:text-gray-400 font-sans leading-normal">
          ...
          <a href="#terms" className="text-primary hover:underline font-semibold transition-colors">Terms of Service</a>{' '}
          and{' '}
          <a href="#privacy" className="text-primary hover:underline font-semibold transition-colors">Privacy Policy</a>.
        </div>
        ...
        <div className="mt-8 text-left text-[0.875rem] text-text-main dark:text-gray-400 font-sans border-t border-neutral-100 dark:border-neutral-800 pt-6">
          ...
          <Link to="/login" className="text-primary font-semibold hover:underline transition-colors">Login</Link>
        </div>
      </div>
    );
  }
      ...
      <div className='flex flex-col w-[389px] h-[472px] gap-[64px]'>
        <div className='flex flex-col gap-[33px]'>
          <div className='flex flex-col gap-[16px]'>
            ...
            <form onSubmit={handleSubmit} className="space-y-7.5">
              <div className='flex flex-col gap-3'>
                  ...
                  <div className="group relative pt-5">
                    <div className="relative">
                      ...
                      <input
                        ...
                        onFocus={() => setFocusedFields(p => ({ ...p, firstName: true }))}
                        onBlur={() => setFocusedFields(p => ({ ...p, firstName: false }))}
                        placeholder=""
                        className={inputStyles('firstName')}
                      />
                      <label className={labelStyles('firstName', firstName)}>First Name</label>
                    </div>
                  ...
                  <div className="group relative pt-5">
                    <div className="relative">
                      ...
                      <input
                        ...
                        onFocus={() => setFocusedFields(p => ({ ...p, lastName: true }))}
                        onBlur={() => setFocusedFields(p => ({ ...p, lastName: false }))}
                        placeholder=""
                        className={inputStyles('lastName')}
                      />
                      <label className={labelStyles('lastName', lastName)}>Last Name</label>
                    </div>
                ...
                <div className="group relative pt-5">
                  <div className="relative">
                    ...
                    />
                    <label className={`${labelStyles('email', email)} flex items-center gap-1.5`}>
                ...
                <div className="group relative pt-5">
                  <div className={`relative w-full h-[46px] rounded-xl border bg-white dark:bg-[#13141a] transition-all duration-200 hover:bg-[#FFF9F2] dark:hover:bg-primary/5 hover:border-primary/50 ${errors.password
                    ...
                    <div className="absolute left-11 right-12 top-1/2 -translate-y-1/2 h-[20px] overflow-hidden">
                      <input
                        ...
                        className={`absolute text-[0.875rem] inset-0 w-full h-full border-none bg-transparent p-0 outline-none focus:outline-none focus:ring-0 text-text-main dark:text-white font-sans text-sm transition-transform ease-in-out duration-300 ${showPassword ? '-translate-y-full' : 'translate-y-0'
                          }`}
                      />
                      ...
                      <input
                        ...
                        aria-hidden={!showPassword}
                        className={`absolute text-[0.875rem] inset-0 w-full h-full border-none bg-transparent p-0 outline-none focus:outline-none focus:ring-0 text-text-main dark:text-white font-sans text-sm transition-transform ease-in-out duration-300 ${showPassword ? 'translate-y-0' : 'translate-y-full'
                          }`}
                      />
                    </div>

                    <label
                      className={`absolute pointer-events-none transition-all duration-350 ease-in-out font-sans select-none z-20 ${focusedFields.password || password !== ''
              ...
              <button
                ...
                className={`w-full py-3.5 px-4 rounded-xl font-semibold text-sm transition-all duration-200 flex items-center justify-center gap-2 ${!isFilled
          ...
          <div className="text-left text-sm text-text-main dark:text-gray-400 font-sans">
            By clicking the button above, you agree to our{' '}
            <a href="#terms" className="text-primary hover:underline font-medium transition-colors">Terms of Service</a>{' '}
            and{' '}
            <a href="#privacy" className="text-primary hover:underline font-medium transition-colors">Privacy Policy</a>.
          </div>
        </div>

        <div className="text-left text-[0.875rem] text-text-main dark:text-gray-400 font-sans">
          Already have an account?{' '}
          <Link to="/login" className="text-primary font-medium hover:underline transition-colors">Login</Link>
        </div>
      </div>
    </div>
  );
