...
export default function VerifyOtp() {
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (otpCode.length < 4) {
      setErrors('Please enter the full 4-digit verification code');
      return;
    }
    setIsLoading(true);
    setErrors('');
    setTimeout(() => {
      setIsLoading(false);
      if (otpCode === '0000') {
        setErrors('Code has expired. Click resend to request a new one.');
      } else {
        setIsSuccess(true);
      }
    }, 1500);
      ...
      <div className="flex flex-col items-center justify-between w-full max-w-[489px] h-[463px] mx-auto bg-white dark:bg-[#181920] border border-[#DDE2E4] dark:border-[#2e303a]/60 rounded-2xl p-6  shadow-[10px_50px_50px_rgba(0,0,0,0.059)] dark:shadow-[10px_50px_50px_rgba(0,0,0,0.25)] transition-all duration-300 text-left">
        ...
        <div className="flex flex-col gap-2 w-[310px] text-center mb-8">
          ...
          <p className="text-[0.875rem] text-text-main dark:text-gray-400 font-sans leading-relaxed">
            We’ve sent an email to <span className="font-semibold text-text-thick dark:text-white">{email}</span> with a an OTP to confirm your account. Check your inbox to  activate your account.
            .
          </p>
        </div>
        ...
        <div className="mt-8 text-left text-[0.875rem] text-text-main dark:text-gray-400 font-sans border-t border-neutral-100 dark:border-neutral-800 pt-6">
          Didn’t get the mail?{' '}
          <button onClick={() => setViewStep('otp')} className="text-primary font-medium hover:underline transition-colors">Resend</button>
        </div>
      </div>
    );
  }
    ...
    <div className="flex items-center justify-center w-full max-w-[445px] h-[424px] mx-auto bg-white dark:bg-[#181920] border border-[#DDE2E4] dark:border-[#2e303a]/60 rounded-2xl p-6 shadow-[10px_50px_50px_rgba(0,0,0,0.059)] dark:shadow-[10px_50px_50px_rgba(0,0,0,0.25)] transition-all duration-300 text-left">
      <div className='flex flex-col gap-[30px] w-[309px] h-[288px]'>
        <form onSubmit={handleSubmit} className="flex flex-col gap-[34px]">
          ...
          <div className='flex flex-col gap-[24px]'>
            <div className="space-y-2">
              ...
              <p className="text-sm text-text-main dark:text-gray-400 font-sans leading-relaxed">
                A four digit OTP code has been sent to your email <span className="font-semibold text-primary dark:text-white">{email}</span>
              </p>
            </div>
            ...
            <div className="space-y-2 w-[272px]">
              <div className="flex gap-4 justify-between" onPaste={handlePaste}>
                {otp.map((digit, index) => (
                  <input
                    ...
                    placeholder='*'
                    className={`w-12.5 h-12.5 text-center text-2xl text-text-main font-bold rounded-xl border bg-white dark:bg-[#13141a] dark:text-white focus:outline-none focus:ring-2 transition-all duration-150 font-sans hover:bg-[#FFF9F2] dark:hover:bg-primary/5 hover:border-primary/50 ${errors
              ...
              {errors && <p className="text-xs text-red-500 font-medium text-left mt-2">{errors}</p>}
            </div>
          </div>
          ...
          <button
            ...
            className={`w-[160px] text-[0.875rem] h-[40px] py-3.5 px-4 rounded-xl font-semibold text-sm transition-all duration-200 flex items-center justify-center gap-2 ${!isOtpFilled
        ...
        <div className="flex text-[0.875rem] items-center gap-1 border-neutral-100 dark:border-neutral-800 mt-6">
          <p className="font-sans text-text-main dark:text-gray-400">Didn’t get the mail?</p>
          <div>
            {timer > 0 ? (
